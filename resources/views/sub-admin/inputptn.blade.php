...
<body class="">
  <div class="wrapper ">
    ...
    <div class="main-panel">
      ...
      <div class="container">
        <h1>LEMBAR PENILAIAN <br> PRAKTIK KERJA LAPANGAN <br> PROGRAM PENDIDIKAN D3/S1 <br> DI BADAN PENGAWAS OBAT DAN MAKANAN RI</h1>
        <main>
            ...
            <form id="nilaiForm" action="{{ route('nilai.store') }}" method="POST">
                @csrf
                  ...
                  <tr>
                    ...
                    <td><input type="text" class="form-control nilai-input" placeholder="Input Nilai" maxlength="3" required></td>
                    @if(isset($kehadiran) && $kehadiran->isNotEmpty())
                        @foreach($kehadiran as $item)
                            <td><input type="text" class="form-control presentase" value="{{ $presentaseKehadiran }}%" readonly></td>
                        @endforeach
                    @else
                        <td>Data kehadiran tidak tersedia</td>
                    @endif
                  ...
                  <div class="button">
                      @if($laporan_akhir->status_pengumpulan === 'sudah dikumpulkan')
                         <button type="button" class="btn2" onclick="if (validateForm()) submitDataAndRedirect()">Input Nilai</button>
